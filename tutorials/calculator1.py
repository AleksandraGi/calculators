...
# https://www.tcl-lang.org/man/tcl8.7/TkCmd/index.html
...
def equals():
    ...
    try:
        ...
        equation_label.set("Syntax eror")
        equation_text = ""
...
label = Label(window, textvariable=equation_label, font=('consolar',20), bg="lightgrey", width=24, height=2)  # It refers to the display box where we display text or image. It can have various options like font, background, foreground, etc. # https://www.geeksforgeeks.org/python/python-tkinter-label/
label.pack()
...
button1.grid(row=0,column=0)    # adding button to frame
...
window.mainloop()   # mainloop() method puts everything on the display, and responds to user input until the program terminates.